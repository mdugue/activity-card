...
- **State** — `hooks/use-carousel.ts` owns the chosen **deck** + selection; the user picks a deck (3 or 4 slides — Intro · details · Wrap-up) rather than adding/removing single slides (`DECKS` in `lib/carousel/types.ts`). A slide is just `{ id, template }`. The chosen **carousel theme** lives in app state separately from the single-card theme.
- **Own theme id space** — carousel themes are keyed by `CarouselThemeId` (`lib/carousel/theme-tokens.ts`), **independent** of the single-card `ThemeId`, so the carousel can grow its own families (e.g. the Dawn/Dusk light·dark pairs) without being capped at the single-card count. Each theme is a different *look*, driven by `heroLayer` (the signature spanning element), `panelKind` (slide layout), `heroMetric` (which number headlines the intro), an optional `crossViz` (wrap-up glyph), plus a `defaultFilter`/`defaultGrain` photo look:
  - **Trace Dawn / Trace Dusk** — route silhouette art-print; a light·serif and a dark·bold variant of the same idea (`heroMetric: distance`, elevation cross-viz).
  - **Ascent Dawn / Ascent Dusk** — elevation mountain-range; light·serif and dark·bold variants (`heroMetric: elevation`, route cross-viz).
  - **Exposure** (photo) — full-bleed photo panorama, magazine masthead, photo-adaptive palette.
  - **Frame** (minimal) — one big datum + a matching sparkline (route / elevation / speed / power) per slide, hairline rules (`panels/frame-panel.tsx`). Supports a background photo, kept clean via text-shadow.
  - **Press** (editorial) — newspaper: masthead, drop cap, pull-quotes (`panels/press-panel.tsx`). Over a photo, text sits in fully-opaque print boxes, not a soft scrim.
  - **Relay** (triathlon) — swim→T1→bike→T2→run timeline spanning the strip (`segments-timeline.tsx`); kept for the multi-sport story (to be expanded).
- **Standard panels** — `components/carousel/templates/` (Hero, StatRow, StatGrid, Editorial) for the route/elevation/photo/segments themes; the signature layer threads through them. The seamless canvas pre-assigns each slide its stats (`planSlideStats`) so the intro headlines the hero number and detail slides page the rest without repeats.
- **Chrome toggles** — athlete name, the "made with effort" wrap-up mark, and page numbers are all **off by default** (`lib/visibility.ts`); the Effort mark and page numbers are carousel-only switches.
- **Shared sidebar** — both editors render `components/app/activity-controls.tsx` (from `control-primitives.tsx`). The theme picker (`theme-picker.tsx`) is shared and generic over the id type, shown below the preview in both modes (carousel labels/order via `CAROUSEL_THEME_LABELS` / `CAROUSEL_THEME_ORDER`). Photo crop/zoom/rotate/mirror/filter/grain reuse `image-adjust-overlay.tsx` + `photo-effects-controls.tsx`, applied deck-wide.
- **Route & spanning layers** — `route-line.tsx` (rounded caps, a graphic checkered finish flag — no coloured GPS pins, poster / desaturated / highlighter, full-width stretch), `elevation-band.tsx` (mountain range), `segments-timeline.tsx` (triathlon legs), `cross-viz.tsx` (wrap-up secondary). Photos render full-bleed with a per-theme default filter + optional film grain and a light/dark legibility veil rather than a heavy overlay.
