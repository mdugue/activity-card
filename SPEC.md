...
- **Route & spanning layers** — `route-line.tsx` (rounded caps, a graphic checkered finish flag — no coloured GPS pins, full-width stretch), `elevation-band.tsx` (mountain range / sparkline), `detail-viz.tsx` (small path + altitude graphics), `cross-viz.tsx` (wrap-up secondary). Photos render full-bleed with a per-theme default filter + optional film grain and a light/dark legibility veil rather than a heavy overlay.
